...
`MediaOps Dashboard`는 광고 운영자가 여러 캠페인의 성과를 빠르게 파악하고,  
...
- `/login`에서 `Admin`, `Manager`, `Viewer` 데모 계정으로 로그인
...
- `Admin`
  ...
  - 운영 메모 수정 가능
  - 일괄 상태 변경 가능
  - Settings 내 관리자 전용 영역 확인 가능
- `Manager`
  - Dashboard / Campaigns / Reports / Settings 접근 가능
  - 캠페인 상태 변경 가능
  - 운영 메모 수정 가능
  - 일괄 상태 변경 가능
- `Viewer`
...
- 페이지네이션
- CSV 다운로드
- 필터 preset 저장 / 불러오기 / 삭제
- 선택 캠페인 일괄 상태 변경
- URL query string 기반 상태 동기화
...
- 운영 메모 작성 / 수정
- 캠페인 상태 변경
- `Overview / Creatives / Revenue / Memo` 탭 분리
...
- Viewer의 읽기 전용 접근 확인
- Admin의 필터 preset 저장/불러오기 및 CSV 다운로드
- Admin의 캠페인 상세 상태 변경 / 메모 저장 흐름 확인
