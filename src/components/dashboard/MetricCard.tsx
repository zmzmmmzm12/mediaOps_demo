...
export function MetricCard({ label, value, delta, tone }: MetricCardProps) {
  ...
  return (
    <article className="surface-card relative min-h-[140px] min-w-0 overflow-hidden p-4">
      <div className="flex h-full flex-col">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[11px] font-medium uppercase tracking-[0.1em] text-[var(--text-tertiary)]">{label}</p>
            <p className="numeric-value mt-3 text-[clamp(1.35rem,5vw,1.75rem)] font-semibold tracking-[-0.04em] text-[var(--text-primary)]">
              {value}
            </p>
          </div>
          <span className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${toneAccentClassMap[tone]}`}>
